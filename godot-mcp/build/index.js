...
class GodotServer {
    ...
    async getDocumentationModule() {
        ...
        this.documentationModule = new DocumentationModule(this.godotPath, undefined, // Use default cache directory
        DEBUG_MODE);
        return this.documentationModule;
    }
    /**
    ...
    async handleRemoveNode(args) {
        ...
        try {
            ...
            return this.createErrorResponse(`Failed to remove node: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleModifyNode(args) {
        ...
        try {
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to modify node: ${stderr}`, [
                    'Check if the node path is correct',
                    'Ensure the node exists in the scene',
            ...
            return this.createErrorResponse(`Failed to modify node: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleQueryNode(args) {
        ...
        try {
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to query node: ${stderr}`, [
                    'Check if the node path is correct',
                    'Ensure the node exists in the scene',
                    'Verify the scene file is valid',
                ]);
            ...
            return this.createErrorResponse(`Failed to query node: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleValidateScript(args) {
        ...
        try {
            ...
            return this.createErrorResponse(`Failed to validate script: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleGetNodeMethods(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.nodeType) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and nodeType']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to get node methods: ${stderr}`, [
                    'Check if the node type is valid',
            ...
            return this.createErrorResponse(`Failed to get node methods: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleCreateSignal(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scriptPath || !args.signalName) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scriptPath, and signalName']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scriptPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to create signal: ${stderr}`, [
                    'Check if the script file exists',
            ...
            return this.createErrorResponse(`Failed to create signal: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleConnectSignal(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
    ...
    async handleListSignals(args) {
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to list signals: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the node exists in the scene',
            ...
            return this.createErrorResponse(`Failed to list signals: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleDisconnectSignal(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.sourceNodePath ||
            !args.signalName || !args.targetNodePath || !args.methodName) {
     ...
     */
    async handleConfigureImport(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.assetPath || !args.importSettings) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, assetPath, and importSettings']);
        }
        ...
        try {
            ...
            if (!existsSync(assetPath)) {
                return this.createErrorResponse(`Asset file does not exist: ${args.assetPath}`, [
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to configure import settings: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to configure import settings: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleAddPhysicsBody(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.bodyType || !args.nodeName || !args.collisionShape) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, bodyType, nodeName, and collisionShape']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to add physics body: ${stderr}`, [
                    'Check if the scene file exists',
            ...
            return this.createErrorResponse(`Failed to add physics body: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleConfigurePhysics(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.nodePath || !args.properties) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, nodePath, and properties']);
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to configure physics: ${stderr}`, [
                    'Check if the scene file exists',
            ...
            return this.createErrorResponse(`Failed to configure physics: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleSetupCollisionLayers(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.nodePath) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, and nodePath']);
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to setup collision layers: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the node path is correct',
            ...
            return this.createErrorResponse(`Failed to setup collision layers: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleCreateUIElement(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.elementType || !args.elementName) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, elementType, and elementName']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to create UI element: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the parent node path is correct',
            ...
            return this.createErrorResponse(`Failed to create UI element: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleApplyTheme(args) {
        ...
        try {
            ...
            if (!existsSync(themePath)) {
                return this.createErrorResponse(`Theme file does not exist: ${args.themePath}`, [
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to apply theme: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the node path is correct',
            ...
            return this.createErrorResponse(`Failed to apply theme: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleSetupLayout(args) {
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to setup layout: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the node path is correct',
            ...
            return this.createErrorResponse(`Failed to setup layout: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleCreateMenu(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.menuName || !args.buttons) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, menuName, and buttons']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        if (!Array.isArray(args.buttons) || args.buttons.length === 0) {
            return this.createErrorResponse('Invalid buttons parameter', ['Provide an array of button definitions with at least one button']);
        }
        try {
            // Check if the project directory exists and contains a project.godot file
            const projectFile = join(args.projectPath, 'project.godot');
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to create menu: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the parent node path is correct',
            ...
            return this.createErrorResponse(`Failed to create menu: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleCreateArea(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.areaType || !args.nodeName || !args.collisionShape) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, areaType, nodeName, and collisionShape']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
     ...
     */
    async handleCreateAnimationPlayer(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and scenePath']);
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to create AnimationPlayer: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the parent node path is correct',
            ...
            return this.createErrorResponse(`Failed to create AnimationPlayer: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleAddKeyframes(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.animationPlayerPath || !args.animationName || !args.track) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, animationPlayerPath, animationName, and track']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to add keyframes: ${stderr}`, [
                    'Check if the scene file exists',
            ...
            return this.createErrorResponse(`Failed to add keyframes: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleSetupAnimationTree(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.animationPlayerPath) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, and animationPlayerPath']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to setup AnimationTree: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the parent node path is correct',
            ...
            return this.createErrorResponse(`Failed to setup AnimationTree: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleAddParticles(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.scenePath || !args.particleType || !args.nodeName) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath, scenePath, particleType, and nodeName']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.scenePath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to add particles: ${stderr}`, [
                    'Check if the scene file exists',
                    'Ensure the parent node path is correct',
            ...
            return this.createErrorResponse(`Failed to add particles: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleGetClassInfo(args) {
        if (!args.className) {
            return this.createErrorResponse('Missing required parameter', ['Provide className']);
        }
        try {
            ...
            return this.createErrorResponse(`Failed to get class info: ${error?.message || 'Unknown error'}`, [
     ...
     */
    async handleGetMethodInfo(args) {
        if (!args.className || !args.methodName) {
            return this.createErrorResponse('Missing required parameters', ['Provide className and methodName']);
        }
        try {
            const docModule = await this.getDocumentationModule();
            const methodInfo = await docModule.getMethodInfo(args.className, args.methodName);
            if (!methodInfo) {
                return this.createErrorResponse(`Method '${args.methodName}' not found in class '${args.className}'`, [
            ...
            return this.createErrorResponse(`Failed to get method info: ${error?.message || 'Unknown error'}`, [
    ...
    async handleSearchDocs(args) {
        if (!args.query) {
            return this.createErrorResponse('Missing required parameter', ['Provide query']);
        }
        try {
            const docModule = await this.getDocumentationModule();
            ...
            return this.createErrorResponse(`Failed to search docs: ${error?.message || 'Unknown error'}`, [
    ...
    async handleGetBestPractices(args) {
        if (!args.topic) {
            return this.createErrorResponse('Missing required parameter', ['Provide topic']);
        }
        try {
            const docModule = await this.getDocumentationModule();
            ...
            return this.createErrorResponse(`Failed to get best practices: ${error?.message || 'Unknown error'}`, [
     ...
     */
    async handleRunWithDebug(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath) {
            return this.createErrorResponse('Missing required parameter', ['Provide projectPath']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Ensure the path does not contain ".." or other suspicious patterns']);
        }
        try {
            // Ensure godotPath is set
            if (!this.godotPath) {
            ...
            return this.createErrorResponse(`Failed to run project with debug: ${error?.message || 'Unknown error'}`, [
     ...
     */
    async handleGetErrorContext(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.errorMessage) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and errorMessage']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Ensure the path does not contain ".." or other suspicious patterns']);
        }
        try {
            ...
            return this.createErrorResponse(`Failed to get error context: ${error?.message || 'Unknown error'}`, [
    ...
    async handleCaptureScreenshot(args) {
        ...
        if (!args.projectPath || !args.outputPath) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and outputPath']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.outputPath)) {
            return this.createErrorResponse('Invalid path', ['Ensure paths do not contain ".." or other suspicious patterns']);
        }
        try {
            // Check if the project directory exists and contains a project.godot file
            const projectFile = join(args.projectPath, 'project.godot');
            if (!existsSync(projectFile)) {
                return this.createErrorResponse(`Not a valid Godot project: ${args.projectPath}`, ['Ensure the path points to a directory containing a project.godot file']);
            }
            ...
            try {
                ...
                if (jsonResult.success) {
                    ...
                    return this.createErrorResponse(`Failed to capture screenshot: ${jsonResult.error || 'Unknown error'}`, [
    ...
    async handleListAssets(args) {
        ...
        try {
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to list assets: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to list assets: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleImportAsset(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.assetPath) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and assetPath']);
        }
        if (!this.validatePath(args.projectPath) || !this.validatePath(args.assetPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('Failed to')) {
                return this.createErrorResponse(`Failed to import asset: ${stderr}`, [
                    'Check if the asset path is correct',
            ...
            return this.createErrorResponse(`Failed to import asset: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleListMissingAssets(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath) {
            return this.createErrorResponse('Missing required parameter', ['Provide projectPath']);
        ...
        try {
            ...
            try {
                const jsonResult = JSON.parse(lastLine);
                if (jsonResult.success) {
                    const report = jsonResult.report;
                    ...
                    return this.createErrorResponse(`Failed to scan for missing assets: ${jsonResult.error || 'Unknown error'}`, [
            ...
            return this.createErrorResponse(`Failed to scan for missing assets: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed and accessible',
                'Check if the project path is correct',
     ...
     */
    async handleUpdateProjectSettings(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to update project settings: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to update project settings: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleConfigureInputMap(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.actions) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and actions']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to configure input map: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to configure input map: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
     ...
     */
    async handleSetupAutoload(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath || !args.autoloads) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and autoloads']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to setup autoload: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to setup autoload: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
    ...
    async handleManagePlugins(args) {
        ...
        if (!args.projectPath || !args.action) {
            return this.createErrorResponse('Missing required parameters', ['Provide projectPath and action']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        ...
        if ((args.action === 'enable' || args.action === 'disable') && !args.pluginName) {
            return this.createErrorResponse('Missing required parameter', ['Provide pluginName for enable/disable actions']);
        }
        try {
            // Check if the project directory exists and contains a project.godot file
            const projectFile = join(args.projectPath, 'project.godot');
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to manage plugins: ${stderr}`, [
            ...
            return this.createErrorResponse(`Failed to manage plugins: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
            ]);
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                godotProcess.on('error', (error) => {
                    ...
                    resolve(this.createErrorResponse(`Failed to run scene: ${error.message}`, [
                        'Ensure Godot is installed correctly',
                        'Check if the GODOT_PATH environment variable is set correctly',
                        'Verify the scene file is valid',
                    ]));
            ...
            return this.createErrorResponse(`Failed to run scene: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
     ...
     */
    async handleRemoteTreeDump(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath) {
            return this.createErrorResponse('Project path is required', ['Provide a valid path to a Godot project directory']);
        ...
        try {
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to dump remote tree: ${stderr}`, [
                    'Check if the scene path is valid (if provided)',
            ...
            try {
                ...
                return this.createErrorResponse(`Failed to parse tree dump result: ${parseError}`, [
            ...
            return this.createErrorResponse(`Failed to dump remote tree: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
     ...
     */
    async handleToggleDebugDraw(args) {
        // Normalize parameters to camelCase
        args = this.normalizeParameters(args);
        if (!args.projectPath) {
            return this.createErrorResponse('Project path is required', ['Provide a valid path to a Godot project directory']);
        ...
        if (!args.mode) {
            return this.createErrorResponse('Debug draw mode is required', ['Specify a valid debug draw mode (e.g., "wireframe", "overdraw", "disabled")']);
        }
        if (!this.validatePath(args.projectPath)) {
            return this.createErrorResponse('Invalid path', ['Provide valid paths without ".." or other potentially unsafe characters']);
        }
        try {
            ...
            if (!validModes.includes(args.mode)) {
                return this.createErrorResponse(`Invalid debug draw mode: ${args.mode}`, [
            ...
            if (stderr && stderr.includes('[ERROR]')) {
                return this.createErrorResponse(`Failed to toggle debug draw: ${stderr}`, [
            ...
            try {
                ...
                return this.createErrorResponse(`Failed to parse toggle debug draw result: ${parseError}`, [
                    'The operation may have failed to execute properly',
                    'Check the Godot console output for errors',
                ]);
            }
            ...
            return this.createErrorResponse(`Failed to toggle debug draw: ${error?.message || 'Unknown error'}`, [
                'Ensure Godot is installed correctly',
                'Check if the GODOT_PATH environment variable is set correctly',
                'Verify the project path is accessible',
