...
func _ready() -> void:
	...
	if create_empty_grid : generate_hex_grid(grid_radius) 
	initialize_astar()
...
func generate_visual_mesh(cells : Array[HexCell]):
	var surface_tool = SurfaceTool.new()
	var mesh = ArrayMesh.new()
	
	surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)
	
	# Generate smooth terrain while snapping to hex grid
	for cell in cells:
	...
	for _cell in cells:
		for i in 6:
			...
			surface_tool.add_index(vert_index + 6) # Center
		vert_index += 7
...
func initialize_from_data(cell_data: Array[HexCell]):
	for cell in cell_data:
		...
		if hex_grid.has(coords):
			hex_grid[coords].terrain_data = cell.terrain_data
			hex_grid[coords].elevation = cell.elevation

	generate_visual_mesh(cell_data)		
	grid_initialized.emit()			
...
func initialize_astar() -> void:
	...
	for cell in hex_grid.values():
		var coords = cell.axial_coords
		var point_id = _get_astar_id(coords.x, coords.y)
		astar.add_point(point_id, cell.global_transform.origin)
...
func _connect_cell_neighbors(cell: HexCell):
	var from_id = _get_astar_id(cell.q, cell.r)
	
	# Get neighbors using axial coordinates
	var neighbors = get_neighbors(cell.q, cell.r)
...
func calculate_directional_cost(source: HexCell, target: HexCell, moving_unit: UnitHandler = null) -> float:
	...
	# Unit-specific modifiers (p.53 TW)
